...
## v0.13.0 거래 전 사실 확인·주문 화면 개선

- PAPER 확인과 최종 LIVE 승인 화면에 현재 메모리의 검증된 데이터만 사용하는 `사전 점검` 영역을 추가했습니다. 시세 신선도, 최우선 매수·매도 호가와 스프레드, 주문 기준가, 추정 주문금액, 매수가능금액 또는 보수적 매도가능수량, 동일 종목·방향 미체결 주문 상태를 `정상`·`주의`·`차단`·`정보 없음`으로 구분합니다.
- MARKET 주문은 신선하고 역전되지 않은 반대편 최우선 호가만 우선 참고합니다. 호가가 오래됐거나 없거나 역전됐다면 최근 체결가를 낮은 신뢰도의 대체값으로 표시합니다. 모든 금액은 추정치이며 체결 보장이나 실제 체결금액 상한이 아닙니다.
- 사전 점검은 기존 risk gate를 보조하는 표시 계층입니다. LIVE 화면의 계좌·미체결 값은 `마지막 확인값`으로 표시되고 `Enter` 이후 기존 preflight가 fresh GET으로 다시 검증합니다. PAPER 확인 화면의 시세 유효시간이 지나면 화면과 확정 callback이 모두 fail-closed로 차단합니다.
- 새로운 차단 임계값·API endpoint·주문 mutation·자동 재시도를 추가하지 않았으며, 브로커 접수와 체결 완료도 계속 구분합니다.
- 주문 모달을 flat·고대비·compact 구조로 정리했습니다. 주문 핵심 → 사실 확인 → 지문/안전 문구 → 격리된 조작 안내 순서이며, 90×30과 140×44 화면에서 수평 스크롤 없이 동작합니다.
...
- 모델은 외부 LLM/API를 호출하지 않는 로컬 k-최근접 이웃(k-NN) 분류기입니다. 현재 메모리의 검증된 공개 캔들에서 1·5·20봉 수익률, 20봉 평균 대비 위치, 봉 변동폭, 상대 거래량을 만들고 과거 유사 패턴 이후의 실제 수익률로 `BUY/HOLD/SELL` 라벨을 학습합니다.
- 검증은 시간순 walk-forward로 수행하며, 예측 horizon 뒤에 확정되는 라벨이 과거 검증에 섞이지 않도록 horizon 길이의 embargo gap을 둡니다. 화면의 `신뢰도`는 유사 패턴 합의도에 이 검증 점수를 곱한 상대 모델 신뢰도이며, 수익 확률이나 통계적으로 보정된 체결 성공 확률이 아닙니다.
- stale/DEGRADED 시세, malformed·non-finite candle, 부족한 표본·클래스 다양성·walk-forward 표본, 균형 정확도 45% 미만에서는 매수·매도를 만들지 않고 `판단 불가`로 fail-closed 처리합니다. 방향 합의나 검증 보정 신뢰도가 부족하면 `관망`으로 낮춥니다.
- AI 판단은 약 2초 간격으로 갱신되며 종목·시간대·최신 candle·stale 상태가 바뀌면 캐시를 즉시 무효화합니다. 공개 가격·거래량 패턴만 사용하므로 뉴스·수수료·슬리피지·기업 이벤트는 반영하지 않습니다.
- 이 계층은 **표시 전용**입니다. 계좌·credential·network·PAPER preview·LIVE plan·order transport를 입력으로 받지 않으며, AI 판단만으로 주문 티켓을 만들거나 `POST /api/v1/orders`를 호출하지 않습니다. 실제 주문은 기존 수동 PAPER/LIVE 경계를 그대로 사용합니다.
