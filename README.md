...
- LIVE/PAPER TUI에서 `p`를 누르면 읽기 전용 포트폴리오 화면이 열립니다. 마스킹 계좌, KRW/USD 매수가능금액, 보유수량·매도가능수량·평단·현재가·매입금액·평가액·평가손익·수익률, 미체결 주문의 총수량·체결수량·잔량을 표시합니다. `r`은 이 화면에서 계좌 정보만 다시 조회하고 `Esc`/`p`로 닫습니다.
- 포트폴리오의 `매수가능`은 브로커가 제공하는 `cashBuyingPower`이며 실제 예수금과 같은 뜻으로 표시하지 않습니다. KRW/USD 평가액과 손익은 환율 없이 통화별로 계산·표시하며 임의로 합산하지 않습니다. 매도가능수량은 보유수량에서 동일 심볼의 활성/알 수 없는 SELL 주문 잔량을 보수적으로 차감한 표시용 값입니다.
- 계좌 REST snapshot은 시장 WebSocket과 독립적으로 30초마다 갱신됩니다. 성공값은 `ACCOUNT FRESH`와 KST 기준시각/경과시간으로 표시하고, 실패하면 마지막 성공값을 유지한 채 `ACCOUNT STALE`로 바뀝니다. 계좌 조회 실패가 시장 연결 상태를 `DEGRADED`로 만들거나 0원/보유 없음으로 위장하지 않습니다.
...
- `p`: 읽기 전용 포트폴리오 열기 (`Esc`/`p` 닫기, 화면 안 `r`은 계좌만 새로고침)
- `1`: 1분봉 차트
- `2`: 5분봉 차트
- `3`: 15분봉 차트
- `4`: 1시간봉 차트
